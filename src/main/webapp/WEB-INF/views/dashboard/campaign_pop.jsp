               ...
               <c:forEach var="rate" items="${ rate }">
				...
				<div class="container3">
						...
						<div id="origin" class="card shadow border-2">
							...
							<div class="card-header">
								<strong class="card-title mb-0">부서별 이행 현황</strong>
							</div>
							<div class="card-header">
								<c:forEach var="fulfilCnt" items="${ fulfilCnt }">
									<c:if test="${ fulfilCnt.fulfil eq '완료' }">
										<strong class="card-title mb-0">${ fulfilCnt.fulfil } ${ fulfilCnt.cnt }건</strong>
									</c:if>
									<c:if test="${ fulfilCnt.fulfil eq '추진중' }">
										<strong class="card-title mb-0">${ fulfilCnt.fulfil } ${ fulfilCnt.cnt }건</strong>
									</c:if>	
									<c:if test="${ fulfilCnt.fulfil eq '보류' }">
										<strong class="card-title mb-0">${ fulfilCnt.fulfil } ${ fulfilCnt.cnt }건</strong>
									</c:if>	
									<c:if test="${ fulfilCnt.fulfil eq '폐기' }">
										<strong class="card-title mb-0">${ fulfilCnt.fulfil } ${ fulfilCnt.cnt }건</strong>
									</c:if>		
								</c:forEach>
							</div>
							<div class="card-body">
								...
								<table class="table table-borderless table-striped">
									...
									<c:forEach var="detailList" items="${ detailList }">
										<tr>
											<th scope="col" style="text-align: center;">${ detailList.section }</th>
											<td style="text-align: center;">${ detailList.manager }</td>
											<td style="text-align: center;">${ detailList.name }</td>
											<td style="text-align: center;">${ detailList.businessPeriod }</td>
											<td style="text-align: center; padding-right: 5%">
												<div class="progress mb-3" style="height: 30px;">
													<div class="progress-bar bg-success" role="progressbar"
														style="width: ${ detailList.rate }%" aria-valuenow="25"
														aria-valuemin="0" aria-valuemax="${ detailList.rate }">${ detailList.rate }</div>
												</div>
											</td>

											<td style="text-align: center;"><c:if
													test="${ detailList.fulfil eq '완료' }">
													<span class="badge badge-pill badge-primary"> ${ detailList.fulfil }
													</span>
												</c:if> <c:if test="${ detailList.fulfil eq '추진중' }">
													<span class="badge badge-pill badge-warning"> ${ detailList.fulfil }
													</span>
												</c:if> <c:if test="${ detailList.fulfil eq '보류' }">
													<span class="badge badge-pill badge-secondary"> ${ detailList.fulfil }
													</span>
												</c:if> <c:if test="${ detailList.fulfil eq '폐기' }">
													<span class="badge badge-pill badge-danger"> ${ detailList.fulfil }
													</span>
												</c:if></td>
										</tr>
										</c:forEach>
